...
\begin{itemize}
    \item \marginnote{6/26:}The sequence $1,1,2,5,14,42,\dots$.
    ...
    \begin{itemize}
        ...
        \begin{figure}[h!]
            ...
            \begin{subfigure}[b]{0.24\linewidth}
                ...
                \begin{tikzpicture}
                    \footnotesize
                    \draw ({90}:1) coordinate (3) node[above]{3}
                        -- ({90-1*360/7}:1) coordinate (4) node[above right]{4}
                        -- ({90-2*360/7}:1) coordinate (5) node[right]{5}
                        -- ({90-3*360/7}:1) coordinate (6) node[below right]{6}
                        -- ({90-4*360/7}:1) coordinate (0) node[below left]{0}
                        -- ({90-5*360/7}:1) coordinate (1) node[left]{1}
                        -- ({90-6*360/7}:1) coordinate (2) node[above left]{2}
                        -- cycle
                    ;
...
\begin{enumerate}
        ...
        \begin{figure}[h!]
            ...
            \begin{subfigure}[b]{0.3\linewidth}
                ...
                \begin{tikzpicture}[scale=0.3]
                    \footnotesize
                    \draw [help lines,dashed] (0,0) grid (8,8);
                    ...
                    \draw [decorate,decoration={brace}] (0,8.3) -- node[above=1mm]{$n+1$} ++(8,0);
                    \draw [decorate,decoration={brace}] (-0.3,0) -- node[left=1mm]{$n+1$} ++(0,8);
            ...
            \begin{subfigure}[b]{0.3\linewidth}
                \centering
                \begin{tikzpicture}[scale=0.3]
                    \foreach \x [evaluate=\x as \y using 8-\x] in {0,...,6} {
                        \fill [rez] (\x,\y) rectangle ([xshift=1cm,yshift=-1cm]\x,\y);
                    }
                    ...
                    \draw [rex,very thick] (0,8) rectangle (7,1);            \draw [rey,very thick] (6,2) rectangle (8,0);
                \end{tikzpicture}
                \caption{Subgrids.}
            ...
            \begin{subfigure}[b]{0.3\linewidth}
                \centering
                \begin{tikzpicture}[scale=0.3]
                    \foreach \x [evaluate=\x as \y using 8-\x] in {0,...,7} {
                        \fill [rez] (\x,\y) rectangle ([xshift=1cm,yshift=-1cm]\x,\y);
                    }
    ...
    \begin{enumerate}
        ...
        \begin{proof}
            ...
            \begin{align*}
                {g_1}^{-1}(g_1g_2) &= {g_1}^{-1}(g_1g_3)\tag*{Multiplicative POE}\\
                ({g_1}^{-1}g_1)g_2 &= ({g_1}^{-1}g_1)g_3\tag*{Associativity}\\
                eg_2 &= eg_3\tag*{Inverse}\\
                g_2 &= g_3\tag*{Identity}
            \end{align*}
            as desired.
        \end{proof}
        ...
        \begin{proof}
            Let $G=\{g_1,g_2,\dots,g_n\}$ be a finite group of order $n$. Consider the row/column defined by $g_i$, where $i$ is an arbitrary natural number between 1 and $n$, inclusive. Suppose for the sake of contradiction that the object $g$ appears twice in the row, such that $g_ig_j=g$ and $g_ig_k=g$, where $j\neq k$. But by part (a), $g_ig_j=g=g_ig_k$ implies that $g_j=g_k$, a contradiction, since every element of $G$ is only listed once in the row/column headers.
        \end{proof}
        \item Prove that for any permutation $\sigma\in\Ss_n$, there exists $N\in\N$ such that $\sigma^N=e$.
        \begin{proof}
            Decompose $\sigma$ into its subcycles, i.e., if there are transpositions (2-cycles), 3-cycles, etc., identify all of these. Let $A$ be the set of subcycle lengths. Let $N$ be the least common multiple of every element of $A$.\par
            Let $a_i$ be the $i^\text{th}$ element of $\sigma$, where $i$ is an arbitrary natural number between 1 and $n$, inclusive. We want to show that the $i^\text{th}$ element of $\sigma^N$ is $i$. Suppose that $i$ is part of a subcycle of length $k$. It follows that the $i^\text{th}$ element of $\sigma^k,\sigma^{2k},\sigma^{3k},\dots$ is $i$. Therefore, since $N=\text{lcm}\,(\{k,\dots\})$, the $i^\text{th}$ element of $\sigma^N$ is $i$, as desired.
        \end{proof}
        \item Prove that for $g\in G$, we have $g^{|G|}=e$.
        \begin{proof}
            Something to do with part (b)? Possibly something with Lagrange's theorem, i.e., elements of subgroups hit $e$ multiple times as the exponent increases while elements not in subgroups cycle through all elements of $G$ before hitting $e$.
        \end{proof}
    \end{enumerate}
    \item \textbf{Erd\H{o}s-Szekeres theorem}:\par
    Prove that for any $n,m\in\N$, every sequence of $nm+1$ distinct real numbers contains an increasing subsequence of length $n+1$ or a decreasing subsequence of length $m+1$.
\end{enumerate}
